...
*Due to **DD1.** in the [Data Model's **Rationale**](module_view_data_model.md/#rationale), question is allowed to be used freely by above layers independently of the adition/removal of types of questions.*
...
- **DLSS1.**
  ...
  *The engagement functionality modules, in the **Analysis and Decision Making** and the **Engagement** layers (for simplicity, both refered to as "**Engagement** layers" from here on) can, as mentioned in **DD2.** in the [Uses View's **Rationale**](module_view_uses.md/#rationale), be treated almost as independent applications in the light of quizzes-tutor's Core Domain. These an also be available on demand. Therefore, a Pluggable Component Framework pattern emerges where each module in the **Engagement** layers maps to a component, pluggable to the interface of the Abstract Core mentioned in **DD2.** in the [Uses View's **Rationale**](module_view_uses.md/#rationale), which is composed of the **Operation**, **Assets** and **Commodities** layers. This is both relevant in the context of the [MS2](system_overview.md#modifiability) and [MS3](system_overview.md#modifiability) **modifiability** scenarios, as it makes both the adition (plugging) and deletion (unplugging) of engagement functionalities straightforward, with minimal risk.*
